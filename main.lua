...
local Config = {
    ...
    WaveSpeed = 38,
    Gravity = 150,
    SpawnPosition = Vector3.new(-40, 2, 0),
    JumpBufferTime = 0.12,
    CoyoteTime = 0.08,
...
function MovementHandler:reset()
    ...
    self.GroundedGraceTimer = Config.CoyoteTime
    self.JumpBufferTimer = 0
...
function MovementHandler:getGroundResult()
    ...
    local probeDistance = halfHeight + Config.GroundRayLength
    local direction = Vector3.new(0, probeDistance * self.ModeHandler.GravityDirection, 0)
    local result = Workspace:Raycast(self.Position, direction, rayParams)

    if result and result.Instance and not result.Instance:GetAttribute("IsHazard") then
        return result
    end
...
function MovementHandler:performJump()
...
function MovementHandler:jump()
    ...
    if mode == "Cube" then
        self.JumpBufferTimer = Config.JumpBufferTime
        if self.IsGrounded or self.GroundedGraceTimer > 0 then
            return self:performJump()
        end
...
function MovementHandler:update(dt)
    local mode = self.ModeHandler.Mode
    local groundResult = self:getGroundResult()
    self.IsGrounded = groundResult ~= nil
    self.JumpBufferTimer = math.max(0, self.JumpBufferTimer - dt)

    if self.IsGrounded then
        self.GroundedGraceTimer = Config.CoyoteTime
    else
        self.GroundedGraceTimer = math.max(0, self.GroundedGraceTimer - dt)
    end
    ...
    if self.JumpBufferTimer > 0 and mode == "Cube" and (self.IsGrounded or self.GroundedGraceTimer > 0) then
        self:performJump()
        groundResult = nil
    end

    if groundResult and (mode == "Cube" or mode == "Ball") then
        local halfHeight = self.PlayerPart.Size.Y / 2
        self.Position = Vector3.new(self.Position.X, groundResult.Position.Y + (halfHeight * -self.ModeHandler.GravityDirection), self.Position.Z)
    end
...
function GameController:onInputBegan(input, processed)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1
        ...
        local didTrigger = self.PlayerController.MovementHandler:jump()
        if didTrigger or self.PlayerController.ModeHandler.Mode == "Cube" then
